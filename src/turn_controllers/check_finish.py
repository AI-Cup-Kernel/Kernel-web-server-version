...
def check_finish(main_game):
    ...
    if main_game.turn_number >= int(main_game.config["number_of_turns"]):
        ...
        if players_strategic_nodes_count.count(max_strategic_nodes) == 1:
            ...
            max_nodes = max(main_game.players, key=lambda player: len(player.nodes))
            game_finished(main_game, main_game.players.index(max_nodes))

    # check if there is a player with enough strategic nodes to win    
    for i in range(len(players_strategic_nodes_count)):
        if players_strategic_nodes_count[i] >= int(main_game.config["number_of_strategic_nodes"]):
...
def game_finished(main_game, winner_id):
    ...
    # generate export 
    export = dict()
    export['node_owners'] = [i.owner.id if i is not None else -1 for i in main_game.nodes]
    export['troop_count'] = [i.number_of_troops for i in main_game.nodes]
